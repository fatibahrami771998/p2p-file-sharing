...
class MenuHandler:
	...
	def serve(choice: str) -> None:
		...
		if choice == "ADFF":
			...
			while True:
				...
				if 0 <= index <= len(temp_files)-1:
					...
					if not LocalData.is_shared_file(temp_files[index]):
						...
						try:
							net_utils.send_packet_and_close(sp_ip4, sp_port, packet)
							shell.print_green(f'Packet sent to supernode: {sp_ip4} [{sp_port}]')
						except net_utils.socket.error:
							shell.print_red(f'\nError while sending packet to supernode: {sp_ip4} [{sp_port}]\n')
			...
			try:
				socket = net_utils.send_packet(sp_ip4, sp_port, packet)

			except (net_utils.socket.error, AttributeError):
				shell.print_red(f'\nError while sending the request to the superpeer: {sp_ip4} [{sp_port}].\n')
			...
			try:
				...
				shell.print_red(f'\nError while receiving the response from the superpeer: {sp_ip4} [{sp_port}].\n')
				return
			except ValueError:
				shell.print_red(f'\nInvalid packet from superpeer: {sp_ip4} [{sp_port}].\n')
				return
			...
			for i in range(num_downloadables):
				...
				for j in range(num_copies):
					try:
						owner_ip4 = net_utils.get_ip_pair(socket.recv(55).decode())
						owner_port = int(socket.recv(5))
						owners.append((owner_ip4, owner_port))
					except net_utils.socket.error:
						shell.print_red(f'\nError while receiving the response from the superpeer: {sp_ip4} [{sp_port}].\n')
						continue
					except ValueError:
			...
			print(f'\nYour current superpeer is: {sp_ip4} [{sp_port}]\n')

		elif choice == "LOGO":
			packet = choice + p_ssid
			sock = net_utils.send_packet(sp_ip4, sp_port, packet)
