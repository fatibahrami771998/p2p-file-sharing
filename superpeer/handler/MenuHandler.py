		...
		if choice == "SUPE":
				...
				if total_db_file == 0:
					...
					for file_row in file_rows:
						...
						for owner_row in owner_rows:
							ip4_peer = owner_row['ip']
							owner_port = int(owner_row['port'])

							# stampa di debug
							# print(f'\n{LocalData.menu_peer_file_index(ip4_peer, peer.port, file.file_md5, file.file_name)}', end='')
							# shell.print_yellow(f' {file.file_md5}', end='')
							# print(f' {file.file_name} from {ip4_peer} on port {peer.port}')
							LocalData.add_menu_peer_file(ip4_peer, owner_port, file_md5, file_name)
							index = LocalData.menu_peer_file_index(ip4_peer, owner_port, file_md5, file_name)
							print(f'{index +1}] ', end='')
							print(f'{file_name} ', end='')
							shell.print_yellow(f'md5={file_md5} ', end='')
							print(f'({ip4_peer} [{owner_port}])')
			...
			while True:
				...
				if 0 <= index <= len(peer_files):
					...
					try:
						Downloader(host_ip4, host_port, packet, file_name).start()
			...
			if not LocalData.get_super_friends():
				...
				for count, friend in enumerate(LocalData.get_super_friends(), 1):
					...
					shell.print_blue(f'{count}] {friend_ip4} {str(friend_port)}')
