...
class MenuTimedResponseHandler(HandlerInterface):
	...
	def serve(self, sd: socket.socket) -> None:
		...
		if command == "AQUE":
			...
			if not LocalData.exist_menu_peer_file(ip4_peer, port_peer, file_md5, filename):
				LocalData.add_menu_peer_file(ip4_peer, port_peer, file_md5, filename)
				index = LocalData.menu_peer_file_index(ip4_peer, port_peer, file_md5, filename)
				print(f'{index +1}] ', end='')
				print(f'{filename} ', end='')
				shell.print_yellow(f'md5={file_md5} ', end='')
				print(f'({ip4_peer} [{port_peer}])')
			...
			if not LocalData.is_super_friend(ip4_peer, port_peer):
				LocalData.add_super_friend(ip4_peer, port_peer)
