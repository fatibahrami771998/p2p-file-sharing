...
class NetworkHandler(HandlerInterface):
	...
	def __forward_packet(self, ip_sender: str, ip4_peer: str, ttl: str, packet: str) -> None:
		...
		if new_ttl > 0:
			...
			recipients = LocalData.get_super_friends_recipients(ip_sender, ip4_peer)
			...
			for superfriend in recipients:
				...
				try:
					net_utils.send_packet_and_close(ip4, port, packet)
					self.log.write_blue(f'Forwarding to {ip4} [{port}] -> ', end='')
					self.log.write(f'{packet}')
				except socket.error as e:
					self.log.write_red(f'Unable to forward a packet to {ip4} [{port}]: {e}')
	...
	def __broadcast_packet(self, packet: str) -> None:
		...
		for superfriend in LocalData.get_super_friends():
			...
			try:
				net_utils.send_packet_and_close(ip4, port, packet)
				self.log.write_blue(f'Broadcasting to {ip4} [{port}] -> ', end='')
				self.log.write(f'{packet}')
			except socket.error as e:
				self.log.write_red(f'Unable to broadcast a packet to {ip4} [{port}]: {e}')
	...
	def serve(self, sd: socket.socket) -> None:
		...
		if command == "SUPE":
			...
			try:
				net_utils.send_packet_and_close(ip4_peer, port_peer, response)
				self.log.write_blue(f'Sending to {ip4_peer} [{port_peer}] -> ', end='')
				self.log.write(f'{response}')
			except socket.error as e:
				self.log.write_red(f'An error has occurred while sending an {response} to {ip4_peer} [{port_peer}]: {e}')
				...
				if total_file != 0:
					...
					for file_row in file_rows:
						...
						for owner_row in owner_rows:
							...
							try:
								net_utils.send_packet_and_close(ip4_peer, port_peer, response)
								self.log.write_blue(f'Sending {ip4_peer} [{port_peer}] -> ', end='')
								self.log.write(f'{response}')
							except socket.error as e:
